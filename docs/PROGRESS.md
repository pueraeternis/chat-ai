...
**Summary:** Documented target architecture for an OpenAI-compatible **chat-proxy** (system `web_search`, client `function` tools, optional `reasoning`), **Qwen3-VL-30B-A3B-Instruct** on vLLM, and embedded **web-search** MCP. Implementation not started; vLLM + Open WebUI run without proxy (plan 01).
